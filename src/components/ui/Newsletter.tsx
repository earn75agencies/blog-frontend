...
const Newsletter = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t('messages.subscribedSuccess'));
      setEmail('');
    } catch (error: any) {
      toast.error(error.response?.data?.message || t('messages.subscribeFailed'));
    } finally {
  ...
  return (
    <div className="card bg-gradient-to-br from-primary-500 to-primary-700 text-white">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-xl font-bold">{t('messages.newsletter')}</h3>
      </div>
      <p className="text-primary-100 mb-4">
        {t('messages.newsletterDesc')}
      </p>
      <form onSubmit={handleSubmit}>
        ...
        >
          {isSubmitting ? t('messages.subscribing') : t('footer.subscribe')}
        </button>
      </form>
    </div>
  );
